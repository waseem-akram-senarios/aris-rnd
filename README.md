...
| **MCP**       | 8503 | Model Context Protocol server. Exposes 5 tools so AI agents (Claude, Cursor, etc.) can search, manage documents/chunks/indexes, and view system stats. |
...
| Tool            | Purpose                                                                        |
|-----------------|--------------------------------------------------------------------------------|
| **search**      | Search documents with modes: `quick` (fast) \| `research` (deep) \| `search` (custom) |
| **documents**   | Full document lifecycle — docs (`list` \| `get` \| `create` \| `update` \| `delete`), chunks (`list_chunks` \| `get_chunk` \| `create_chunk` \| `update_chunk` \| `delete_chunk`), indexes (`list_indexes` \| `index_info` \| `delete_index`) |
| **system_info** | System statistics (documents, queries, costs, languages)                       |
